...
#--4) Simulate Microcensus data in some randomly selected clusters across 6 provinces of the 26 provinces 
#--5) Calculate scale factor where there are Microcensus overlap with the pop counts
#--6) Fit scale factor model at cluster level and predict across the entire grid cells where no overlap exists 
#--7) Fit pop model at cluster level and predict across the entire grid cells for both scaled and unscaled
#--8) Carry out model checks and cross validations, aggregate and compare
#--9) Withhold data for a couple of provinces and refit 
#--10) Predict pop count for all grid cells including the unsampled provinces for bath scaled and unscaled
#--11) Repeat 1 - 10 for a p% biased population with each cluster biased with b% for p and b in 0.1 to 1.
...
out_path <- paste0(path , "/Tanganyinka/paper/simstudy/output")
...
rmae <- ggpar(plot_mae, 
              ...
              legend.title="Proportion of biased population (%)",
              font.x = c(16),
              font.y = c(16),
              font.legend = c(16),
...
#plot_bias <- ggplot(bias, aes(x=bld_cover, y=estimate, color = pop_cover))+
#geom_point()+
#geom_line()+
#facet_wrap(~method)
plot_bias <- ggline(bias, x = "mag_bias", y = "estimate",
                    error.plot = "estimate",
                    facet.by = "method",
                    panel.labs= list(method=c("Scaled", "Unscaled")),
                    color = "pop_bias2", 
...
#geom_line()+
#facet_wrap(~method)
plot_corr <- ggline(corr, x = "mag_bias", y = "estimate",
                    error.plot = "estimate",
                    facet.by = "method",
                    panel.labs= list(method=c("Scaled", "Unscaled")),
                    color = "pop_bias2", 
...
rrcorr <- ggpar(plot_corr, 
                ...
                ylab = "Correlation(Obs versus pred)") 
rrcorr 
...
### MAE
met_mae <- met_long[met_long$metric=="MAE",]
barp <- met_mae %>%
  ggplot( aes(x=mag_bias, y=estimate, fill=pop_bias2)) +
  #ggplot(data = df_continent, aes(x = reorder(continent, -mean_temp), y = mean_temp, fill = as.factor(Year))) +
  #geom_bar(stat = "identity", color = "blue", 
        #   position = position_dodge()) +
  geom_boxplot()+
  #geom_line( linetype= 2, col="dark blue", size = 0.7) +
...
rcval5 <-  ggpar(barp, xlab="Magnitude of bias (%)", ylab="MAE",
                 legend = "top", legend.title=element_text("Proportion of biased \n population (%):"),
                 font.legend=c(20),
                 palette = c("aaas"),
...
ggarrange(rrmse, rcval5,nrow = 2, ncol=1)


### Reduction in relative bias
rrbias_dat <- as.data.frame(read.csv(paste0(out_path, "/reduction_in_bias.csv")))
rrbias_dat <- rrbias_dat %>% drop_na(rrbias)%>% dplyr::select(c(mag_bias,
                                                         pop_bias,
                                                         rrbias))

rrbias_dat$rrbias <- round(rrbias_dat$rrbias*100, 2)
rrbias_dat$pop_bias <- factor(rrbias_dat$pop_bias)
rrbias_dat$mag_bias <- factor(rrbias_dat$mag_bias,
                              labels=c("10% Bias", "20% Bias",
                              "30% Bias", "40% Bias",
                              "50% Bias", "60% Bias",
                              "70% Bias", "80% Bias",
                              "90% Bias", "100% Bias"))

levels(rrbias_dat$mag_bias)
barp <- rrbias_dat %>%
  ggplot( aes(x=pop_bias, y=rrbias, fill=mag_bias)) +
...
rcval5 <-  ggpar(barp, xlab="Proportion of biased population(%)", 
                 ylab="Reduction in relative bias(%)",
...
save.image(paste0(results_path,"/tan_sim_study.Rdata"))